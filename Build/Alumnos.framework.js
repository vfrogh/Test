...
function _VideoPlayerWebGL_CreateVideo(url, cors, id, autoplay, loop, muted, volume, pan, forceMono, playbackSpeed, texturePtr, events, canUpdateTexture, useMipMap) {
 id = UTF8ToString(id);
 ...
 url = UTF8ToString(url) + "#t=0.0000001";
  ...
  if (!video.VideoPlayerWebGL.createdVideoFlag && video.VideoPlayerWebGL.playingFlag && video.VideoPlayerWebGL.timeupdateFlag) {
   video.addEventListener("timeupdate", function() {
    ...
    (function(number, bitPosition) {
     if (Module["VideoPlayerWebGL"].canUpdateTextureArr.byteLength === 0) Module["VideoPlayerWebGL"].canUpdateTextureArr = new Uint32Array(buffer, Module["VideoPlayerWebGL"].canUpdateTextureRef, 1);
 ...
 video.VideoPlayerWebGL.seeked = function() {
  video.VideoPlayerWebGL.canUpdateTexture = true;
  _VideoPlayerWebGL_UpdateTexture(id);
  video.VideoPlayerWebGL.canUpdateTexture = false;
 };
...
function _VideoPlayerWebGL_DestroyVideo(id) {
 id = UTF8ToString(id);
 ...
 video.load();
 Object.keys(video.VideoPlayerWebGL.events).forEach(function(event) {
  video.removeEventListener(event, video.VideoPlayerWebGL.events[event]);
 });
...
function _VideoPlayerWebGL_GetTimeVideo(id) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 return !video ? -1 : video.currentTime;
}

function _VideoPlayerWebGL_HeightVideo(id) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 return !video ? 0 : video.videoHeight;
}

function _VideoPlayerWebGL_IsPausedVideo(id) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 return !video ? false : video.paused;
}

function _VideoPlayerWebGL_IsPlayingVideo(id) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 return !video ? false : !!(video.currentTime > 0 && !video.paused && !video.ended && video.readyState > 2);
}

function _VideoPlayerWebGL_IsReadyVideo(id) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
 return video.readyState >= 4;
}

function _VideoPlayerWebGL_LengthVideo(id) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 return !video ? -1 : video.duration;
}

function _VideoPlayerWebGL_PauseVideo(id) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
 video.pause();
}

function _VideoPlayerWebGL_PlayBackSpeedVideo(id, speed) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
...
function _VideoPlayerWebGL_PlayVideo(id) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
 video.play();
}

function _VideoPlayerWebGL_PlayVideoPointerDown(id) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
...
function _VideoPlayerWebGL_RegisterEvent(id, events) {
 id = UTF8ToString(id);
...
function _VideoPlayerWebGL_RestartVideo(id) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
 video.currentTime = 0;
...
function _VideoPlayerWebGL_SetAutoplayVideo(id, autoplay) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
 if (autoplay && !video.hasAttribute("autoplay")) video.setAttribute("autoplay", ""); else if (!autoplay && video.hasAttribute("autoplay")) video.removeAttribute("autoplay");
}

function _VideoPlayerWebGL_SetCorsVideo(id, cors) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
 cors = UTF8ToString(cors);
...
function _VideoPlayerWebGL_SetForceMonoVideo(id, forceMono) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
 video.forceMono = forceMono;
}

function _VideoPlayerWebGL_SetLoopVideo(id, loop) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
 if (loop && !video.hasAttribute("loop")) video.setAttribute("loop", ""); else if (!loop && video.hasAttribute("loop")) video.removeAttribute("loop");
}

function _VideoPlayerWebGL_SetMuteVideo(id, mute) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
 video.muted = mute;
}

function _VideoPlayerWebGL_SetPanVideo(id, pan) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
 Module["VideoPlayerWebGL"].setPan(video, pan);
}

function _VideoPlayerWebGL_SetTimeVideo(id, time) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
...
function _VideoPlayerWebGL_SetVolumeVideo(id, volume) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
 video.volume = volume;
}

function _VideoPlayerWebGL_SourceVideo(id, url, cors) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
...
function _VideoPlayerWebGL_StopVideo(id) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
 video.pause();
 video.currentTime = 0;
...
function _VideoPlayerWebGL_UnregisterEvent(id, events) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
 events = UTF8ToString(events).replace(/\s/g, "").split(",");
 events = events[0] === "-1" ? [ "canplay", "canplaythrough", "complete", "durationchange", "emptied", "ended", "loadeddata", "loadedmetadata", "pause", "play", "playing", "progress", "ratechange", "seeked", "seeking", "stalled", "suspend", "timeupdate", "volumechange", "waiting" ] : events[0] === "0" ? undefined : events;
...
function _VideoPlayerWebGL_UpdateTexture(id) {
 const video = document.getElementById("VideoPlayerWebGL_" + id);
...
function _VideoPlayerWebGL_WidthVideo(id) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 return !video ? 0 : video.videoWidth;
}

function _VideoPlayerWebGL_unlockVideoPlayback(id) {
 const video = document.getElementById("VideoPlayerWebGL_" + UTF8ToString(id));
 if (!video) return;
 document.documentElement.addEventListener("pointerup", function() {
...
var asmLibraryArg = {
 ...
 "VideoPlayerWebGL_CreateVideo": _VideoPlayerWebGL_CreateVideo,
 "VideoPlayerWebGL_DestroyVideo": _VideoPlayerWebGL_DestroyVideo,
 "VideoPlayerWebGL_GetTimeVideo": _VideoPlayerWebGL_GetTimeVideo,
 "VideoPlayerWebGL_HeightVideo": _VideoPlayerWebGL_HeightVideo,
 "VideoPlayerWebGL_IsPausedVideo": _VideoPlayerWebGL_IsPausedVideo,
 "VideoPlayerWebGL_IsPlayingVideo": _VideoPlayerWebGL_IsPlayingVideo,
 "VideoPlayerWebGL_IsReadyVideo": _VideoPlayerWebGL_IsReadyVideo,
 "VideoPlayerWebGL_LengthVideo": _VideoPlayerWebGL_LengthVideo,
 "VideoPlayerWebGL_PauseVideo": _VideoPlayerWebGL_PauseVideo,
 "VideoPlayerWebGL_PlayBackSpeedVideo": _VideoPlayerWebGL_PlayBackSpeedVideo,
 "VideoPlayerWebGL_PlayVideo": _VideoPlayerWebGL_PlayVideo,
 "VideoPlayerWebGL_PlayVideoPointerDown": _VideoPlayerWebGL_PlayVideoPointerDown,
 "VideoPlayerWebGL_RegisterEvent": _VideoPlayerWebGL_RegisterEvent,
 "VideoPlayerWebGL_RestartVideo": _VideoPlayerWebGL_RestartVideo,
 "VideoPlayerWebGL_SetAutoplayVideo": _VideoPlayerWebGL_SetAutoplayVideo,
 "VideoPlayerWebGL_SetCorsVideo": _VideoPlayerWebGL_SetCorsVideo,
 "VideoPlayerWebGL_SetForceMonoVideo": _VideoPlayerWebGL_SetForceMonoVideo,
 "VideoPlayerWebGL_SetLoopVideo": _VideoPlayerWebGL_SetLoopVideo,
 "VideoPlayerWebGL_SetMuteVideo": _VideoPlayerWebGL_SetMuteVideo,
 "VideoPlayerWebGL_SetPanVideo": _VideoPlayerWebGL_SetPanVideo,
 "VideoPlayerWebGL_SetTimeVideo": _VideoPlayerWebGL_SetTimeVideo,
 "VideoPlayerWebGL_SetVolumeVideo": _VideoPlayerWebGL_SetVolumeVideo,
 "VideoPlayerWebGL_SourceVideo": _VideoPlayerWebGL_SourceVideo,
 "VideoPlayerWebGL_StopVideo": _VideoPlayerWebGL_StopVideo,
 "VideoPlayerWebGL_UnregisterEvent": _VideoPlayerWebGL_UnregisterEvent,
 "VideoPlayerWebGL_UpdateTexture": _VideoPlayerWebGL_UpdateTexture,
 "VideoPlayerWebGL_WidthVideo": _VideoPlayerWebGL_WidthVideo,
 "VideoPlayerWebGL_unlockVideoPlayback": _VideoPlayerWebGL_unlockVideoPlayback,
...
var _VideoPlayerWebGL_canplayClbks = Module["_VideoPlayerWebGL_canplayClbks"] = createExportWrapper("VideoPlayerWebGL_canplayClbks");

var _VideoPlayerWebGL_canplaythroughClbks = Module["_VideoPlayerWebGL_canplaythroughClbks"] = createExportWrapper("VideoPlayerWebGL_canplaythroughClbks");

var _VideoPlayerWebGL_completeClbks = Module["_VideoPlayerWebGL_completeClbks"] = createExportWrapper("VideoPlayerWebGL_completeClbks");

var _VideoPlayerWebGL_durationchangeClbks = Module["_VideoPlayerWebGL_durationchangeClbks"] = createExportWrapper("VideoPlayerWebGL_durationchangeClbks");

var _VideoPlayerWebGL_emptiedClbks = Module["_VideoPlayerWebGL_emptiedClbks"] = createExportWrapper("VideoPlayerWebGL_emptiedClbks");

var _VideoPlayerWebGL_endedClbks = Module["_VideoPlayerWebGL_endedClbks"] = createExportWrapper("VideoPlayerWebGL_endedClbks");

var _VideoPlayerWebGL_loadeddataClbks = Module["_VideoPlayerWebGL_loadeddataClbks"] = createExportWrapper("VideoPlayerWebGL_loadeddataClbks");

var _VideoPlayerWebGL_loadedmetadataClbks = Module["_VideoPlayerWebGL_loadedmetadataClbks"] = createExportWrapper("VideoPlayerWebGL_loadedmetadataClbks");

var _VideoPlayerWebGL_pauseClbks = Module["_VideoPlayerWebGL_pauseClbks"] = createExportWrapper("VideoPlayerWebGL_pauseClbks");

var _VideoPlayerWebGL_playClbks = Module["_VideoPlayerWebGL_playClbks"] = createExportWrapper("VideoPlayerWebGL_playClbks");

var _VideoPlayerWebGL_playingClbks = Module["_VideoPlayerWebGL_playingClbks"] = createExportWrapper("VideoPlayerWebGL_playingClbks");

var _VideoPlayerWebGL_progressClbks = Module["_VideoPlayerWebGL_progressClbks"] = createExportWrapper("VideoPlayerWebGL_progressClbks");

var _VideoPlayerWebGL_ratechangeClbks = Module["_VideoPlayerWebGL_ratechangeClbks"] = createExportWrapper("VideoPlayerWebGL_ratechangeClbks");

var _VideoPlayerWebGL_seekedClbks = Module["_VideoPlayerWebGL_seekedClbks"] = createExportWrapper("VideoPlayerWebGL_seekedClbks");

var _VideoPlayerWebGL_seekingClbks = Module["_VideoPlayerWebGL_seekingClbks"] = createExportWrapper("VideoPlayerWebGL_seekingClbks");

var _VideoPlayerWebGL_stalledClbks = Module["_VideoPlayerWebGL_stalledClbks"] = createExportWrapper("VideoPlayerWebGL_stalledClbks");

var _VideoPlayerWebGL_suspendClbks = Module["_VideoPlayerWebGL_suspendClbks"] = createExportWrapper("VideoPlayerWebGL_suspendClbks");

var _VideoPlayerWebGL_timeupdateClbks = Module["_VideoPlayerWebGL_timeupdateClbks"] = createExportWrapper("VideoPlayerWebGL_timeupdateClbks");

var _VideoPlayerWebGL_volumechangeClbks = Module["_VideoPlayerWebGL_volumechangeClbks"] = createExportWrapper("VideoPlayerWebGL_volumechangeClbks");

var _VideoPlayerWebGL_waitingClbks = Module["_VideoPlayerWebGL_waitingClbks"] = createExportWrapper("VideoPlayerWebGL_waitingClbks");
...
var dynCall_viiiiiiddidiiiii = Module["dynCall_viiiiiiddidiiiii"] = createExportWrapper("dynCall_viiiiiiddidiiiii");

var dynCall_vidi = Module["dynCall_vidi"] = createExportWrapper("dynCall_vidi");

var dynCall_viiiiiiiddidiii = Module["dynCall_viiiiiiiddidiii"] = createExportWrapper("dynCall_viiiiiiiddidiii");
